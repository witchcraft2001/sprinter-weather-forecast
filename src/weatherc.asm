...
; CONFIG_LOAD has completely consumed CFG_FILE_BUFFER before GOPHER_FETCH
; starts. Reuse its first half for the receive chunk instead of reserving a
; second 512-byte area in the single resident WIN2 page.
RECV_BUFFER     EQU CFG_FILE_BUFFER
RECV_BUFFER_SIZE EQU 512
RESPONSE_SIZE   EQU REQUEST_SIZE + 2
; Remaining transport scratch shares the unused portion of CFG_FILE_BUFFER.
; RECV occupies 0..511 and the WX1 staging model occupies 512..811.
RESPONSE_TAIL   EQU CFG_FILE_BUFFER + 816
TRANSPORT_INPUT_PTR EQU RESPONSE_SIZE + 2
...
TRANSPORT_DETAIL EQU CFG_FILE_BUFFER + 832
TRANSPORT_DETAIL_SIZE EQU 128
LAST_RECV_LENGTH EQU CFG_FILE_BUFFER + 820
TRANSPORT_RESPONSE_SIZE EQU CFG_FILE_BUFFER + 822
; The config line buffer is likewise dead once network parsing begins.
WX1_LINE_BUFFER EQU CFG_LINE_BUFFER
WX1_LINE_LEN    EQU TRANSPORT_DETAIL_STATUS + 1
...
; The unpublished model only lives during WX1 parsing, after CONFIG_LOAD.
; CFG_FILE_BUFFER remains large enough for it and later becomes formatting
; scratch once the validated model has been committed.
WX1_STAGE_MODEL EQU CFG_FILE_BUFFER + 512
WX1_MODEL       EQU WX1_DATE_YEAR + 2
        IFDEF WEATHER_GRAPHICS
...
BSS_SIZE        EQU BSS_END - BSS_BASE
        ...
        ASSERT  RECV_BUFFER + RECV_BUFFER_SIZE <= WX1_STAGE_MODEL
        ASSERT  WX1_STAGE_MODEL + WM_MODEL_SIZE <= RESPONSE_TAIL
        ASSERT  RESPONSE_TAIL + 4 <= LAST_RECV_LENGTH
        ASSERT  LAST_RECV_LENGTH + 2 <= TRANSPORT_RESPONSE_SIZE
        ASSERT  TRANSPORT_RESPONSE_SIZE + 2 <= TRANSPORT_DETAIL
