...
`WEATHER.EXE` — графическая версия 320×256. При запуске она считывает встроенные
ассеты в пять DSS-страниц и сразу закрывает EXE; дальнейшая перерисовка не
зависит от диска. Для неё рядом с EXE нужны `AFNT320.DLL` и `GFX320.DLL`.
