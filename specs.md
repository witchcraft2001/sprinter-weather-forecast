...
Версия: 0.1.1. Автор: Dmitry Mikhalchenkov, FidoNet: 2:5030/1997.10.
...
Текущий экран показывает заголовок, location/country, время обновления,
...
активный backend. `R`/Enter выполняет полный refresh с перечитыванием CFG и
`NET`, F1 через fade открывает встроенную помощь: backend указан рядом с версией,
а FidoNet-адрес — рядом с автором. Esc завершает программу. Возврат из помощи также выполняется через
fade. Палитра включается fade-in, при выходе используется fade-out. Графический режим
